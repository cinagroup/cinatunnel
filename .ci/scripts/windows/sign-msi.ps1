...
$msiFiles = Get-ChildItem -Path $ARTIFACT_DIR -Filter "cinatunnel-windows-*.msi" -ErrorAction SilentlyContinue
...
foreach ($file in $msiFiles) {
    ...
    azuresigntool.exe sign -kvu $env:KEY_VAULT_URL -kvi "$env:KEY_VAULT_CLIENT_ID" -kvs "$env:KEY_VAULT_SECRET" -kvc "$env:KEY_VAULT_CERTIFICATE" -kvt "$env:KEY_VAULT_TENANT_ID" -tr "$TIMESTAMP_RFC3161" -d "CinaTunnel Daemon" .\\$ARTIFACT_DIR\\$($file.Name)
}
