...
# build 'cinatunnel-darwin-amd64.tgz'
mkdir -p artifacts
...
BUNDLE_ID="com.cina.cinatunnel"
SEC_DUP_MSG="security: SecKeychainItemImport: The specified item already exists in the keychain."
export PATH="$PATH:/usr/local/bin"
FILENAME="$(pwd)/artifacts/cinatunnel-darwin-$TARGET_ARCH.tgz"
PKGNAME="$(pwd)/artifacts/cinatunnel-$TARGET_ARCH.pkg"
mkdir -p ../src/github.com/cinagroup/    
cp -r . ../src/github.com/cinagroup/cinatunnel
cd ../src/github.com/cinagroup/cinatunnel 
...
create_cinatunnel_build_keychain() {
  ...
  if [ ! -f "$HOME/Library/Keychains/cinatunnel_build_keychain.keychain-db" ]; then
    security create-keychain -p "$PRIVATE_KEY_PASS" cinatunnel_build_keychain
  else
    echo "Keychain already exists: cinatunnel_build_keychain"
  fi

  # Append temp keychain to the user domain
  security list-keychains -d user -s cinatunnel_build_keychain $(security list-keychains -d user | sed s/\"//g)

  # Remove relock timeout
  security set-keychain-settings cinatunnel_build_keychain

  # Unlock keychain so it doesn't require password
  security unlock-keychain -p "$PRIVATE_KEY_PASS" cinatunnel_build_keychain
...
import_private_keys() {
    ...
    if [[ ! -z "$PRIVATE_KEY_ENV_VAR" ]]; then
      if [[ ! -z "$PRIVATE_KEY_PASS" ]]; then
        ...
        # we set || true  here and for every `security import invoke` because the  "duplicate SecKeychainItemImport" error
        # will cause set -e to exit 1. It is okay we do this because we deliberately handle this error in the lines below.
        local out=$(security import ${PRIVATE_KEY_FILE_NAME} -k cinatunnel_build_keychain -P "$PRIVATE_KEY_PASS" -T /usr/bin/pkgbuild -A -P "${PRIVATE_KEY_PASS}" 2>&1) || true
        local exitcode=$?
...
create_cinatunnel_build_keychain "${CFD_CODE_SIGN_PASS}"
...
if [[ ! -z "$CFD_CODE_SIGN_NAME" ]]; then
  ...
  if [[ -n "$(security find-certificate -c "Developer ID Application" cinatunnel_build_keychain | cut -d'"' -f 4 -s | grep "Developer ID Application:" | head -1)" ]]; then
    CODE_SIGN_NAME=$(security find-certificate -c "Developer ID Application" cinatunnel_build_keychain | cut -d'"' -f 4 -s | grep "Developer ID Application:" | head -1)
  else
    CODE_SIGN_NAME=""
  fi
fi
...
if [[ ! -z "$CFD_INSTALLER_NAME" ]]; then
  ...
  if [[ -n "$(security find-certificate -c "Developer ID Installer" cinatunnel_build_keychain | cut -d'"' -f 4 -s | grep "Developer ID Installer:" | head -1)" ]]; then
    PKG_SIGN_NAME=$(security find-certificate -c "Developer ID Installer" cinatunnel_build_keychain | cut -d'"' -f 4 -s | grep "Developer ID Installer:" | head -1)
  else
    PKG_SIGN_NAME=""
  fi
fi
...
GOCACHE="$PWD/../../../../" GOPATH="$PWD/../../../../" CGO_ENABLED=1 make cinatunnel
...
if [[ ! -z "$CFD_CODE_SIGN_PASS" ]]; then
  security set-key-partition-list -S apple-tool:,apple: -s -k "${CFD_CODE_SIGN_PASS}" cinatunnel_build_keychain
fi

# sign the cinatunnel binary
if [[ ! -z "$CODE_SIGN_NAME" ]]; then
  codesign --keychain $HOME/Library/Keychains/cinatunnel_build_keychain.keychain-db -s "${CODE_SIGN_NAME}" -fv --options runtime --timestamp ${BINARY_NAME}
...
security delete-keychain cinatunnel_build_keychain
