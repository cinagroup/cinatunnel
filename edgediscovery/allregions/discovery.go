...
const (
	...
	dotServerName = "cinatunnel-dns.com"
...
var friendlyDNSErrorLines = []string{
	...
	`     https://docs.cinatunnel.com/1.1.1.1/setting-up-1.1.1.1/`,
}
...
func edgeDiscovery(log *zerolog.Logger, srvService string) ([][]*EdgeAddr, error) {
	logger := log.With().Int(management.EventTypeKey, int(management.Cinad)).Logger()
	logger.Debug().
		Int(management.EventTypeKey, int(management.Cinad)).
...
func ResolveAddrs(addrs []string, log *zerolog.Logger) (resolved []*EdgeAddr) {
	for _, addr := range addrs {
		...
		if err != nil {
			log.Error().Int(management.EventTypeKey, int(management.Cinad)).
