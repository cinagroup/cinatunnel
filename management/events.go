...
// Example: assigning LogEventType.Cinad to a zerolog event will allow the client to filter for only
// the Cinad-related events.
type LogEventType int8

const (
	// Cinad events are significant to cinatunnel operations like connection state changes.
	// Cinad is also the default event type for any events that haven't been separated into a proper event type.
	Cinad LogEventType = iota
	HTTP
	TCP
	UDP
)
