...
import (
	...
	"github.com/cinagroup/cinatunnel/overwatch"
	"github.com/cinagroup/cinatunnel/token"
	"github.com/cinagroup/cinatunnel/tracing"
	"github.com/cinagroup/cinatunnel/watcher"
)
...
func main() {
	...
	app := &cli.App{}
	app.Name = "cinatunnel"
	app.Usage = "Cina's command-line tool and agent"
	app.UsageText = "cinatunnel [global options] [command] [command options]"
	app.Copyright = fmt.Sprintf(
		`(c) %d Cina Inc.
   Your installation of cinatunnel software constitutes a symbol of your signature indicating that you accept
   the terms of the Apache License Version 2.0 (https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/license),
   Terms (https://www.cinatunnel.com/terms/) and Privacy Policy (https://www.cinatunnel.com/privacypolicy/).`,
		time.Now().Year(),
	)
	app.Version = fmt.Sprintf("%s (built %s%s)", Version, BuildTime, bInfo.GetBuildTypeMsg())
	app.Description = `cinatunnel connects your machine or user identity to Cina's global network.
	You can use it to authenticate a session to reach an API behind Access, route web traffic to this machine,
	and configure access control.

	See https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps for more in-depth documentation.`
	app.Flags = flags()
...
// cinatunnel was started without any flags
