...
import (
	...
	"github.com/cinagroup/cinatunnel/sshgen"
...
{{- if .ShortLivedCerts}}
Match host {{.Hostname}} exec "{{.Cinatunnel}} access ssh-gen --hostname %h"
  ProxyCommand {{.Cinatunnel}} access ssh --hostname %h
  IdentityFile ~/.cinatunnel/%h-cf_key
  CertificateFile ~/.cinatunnel/%h-cf_key-cert.pub
{{- else}}
Host {{.Hostname}}
  ProxyCommand {{.Cinatunnel}} access ssh --hostname %h
{{end}}
	...
	return []*cli.Command{
		{
			...
			Description: `Cina Access protects internal resources by securing, authenticating and monitoring access
			per-user and by application. With Cina Access, only authenticated users with the required permissions are
			...
			Subcommands: []*cli.Command{
				{
					...
					administrator. cinatunnel stores the token in local storage.`,
					Flags: []cli.Flag{
						&cli.BoolFlag{
							Name:    loginQuietFlag,
							Aliases: []string{"q"},
				...
				},
				{
					...
					Usage:       "",
					ArgsUsage:   "",
					Description: `The tcp subcommand sends data over a proxy to the Cina edge.`,
					Flags: []cli.Flag{
						&cli.StringFlag{
						...
						&cli.StringFlag{
							...
							Usage:   "specify the host:port to forward data to Cina edge.",
...
func sshConfig(c *cli.Context) error {
	...
	return t.Execute(os.Stdout, config{Home: os.Getenv("HOME"), ShortLivedCerts: genCertBool, Hostname: hostname, Cinatunnel: cinatunnelPath()})
}
...
// cinatunnelPath pulls the full path of cinatunnel on disk
func cinatunnelPath() string {
	...
	for _, p := range strings.Split(os.Getenv("PATH"), ":") {
		path := fmt.Sprintf("%s/%s", p, "cinatunnel")
		if isFileThere(path) {
			return path
		}
	}
...
func isTokenValid(options *carrier.StartOptions, log *zerolog.Logger) (bool, error) {
	...
	query := req.URL.Query()
	query.Set("cinatunnel_token_check", "true")
