...
{{- if .ShortLivedCerts}}
Match host {{.Hostname}} exec "{{.Cinad}} access ssh-gen --hostname %h"
  ProxyCommand {{.Cinad}} access ssh --hostname %h
...
func sshConfig(c *cli.Context) error {
	...
	return t.Execute(os.Stdout, config{Home: os.Getenv("HOME"), ShortLivedCerts: genCertBool, Hostname: hostname, Cinad: cinatunnelPath()})
}
