...
func buildTailManagementTokenSubcommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:   "cinatunnel tail token TUNNEL_ID",
...
func buildTailCommand(subcommands []*cli.Command) *cli.Command {
	return &cli.Command{
		Name:      "tail",
		Action:    Run,
		Usage:     "Stream logs from a remote cinatunnel",
		UsageText: "cinatunnel tail [tail command options] [TUNNEL-ID]",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:    "connector-id",
				Usage:   "Access a specific cinatunnel instance by connector id (for when a tunnel has multiple cinatunnel's)",
				Value:   "",
				EnvVars: []string{"TUNNEL_MANAGEMENT_CONNECTOR"},
			},
			&cli.StringSliceFlag{
				Name:    "event",
				Usage:   "Filter by specific Events (cinatunnel, http, tcp, udp) otherwise, defaults to send all events",
...
func handleValidationError(resp *http.Response, log *zerolog.Logger) {
	if resp.StatusCode == 530 {
		log.Error().Msgf("no cinatunnel connector available or reachable via management request (a recent version of cinatunnel is required to use streaming logs)")
	}
...
func parseFilters(c *cli.Context) (*management.StreamingFilters, error) {
	...
	for _, v := range argEvents {
		...
		if !ok {
			return nil, fmt.Errorf("invalid --event filter provided, please use one of the following EventTypes: cinatunnel, http, tcp, udp")
		}
