...
const fedRampBaseApiURL = "https://api.fed.cinatunnel.com/client/v4"
	...
	if err = json.Unmarshal(body, &credentials); err != nil {
		if filepath.Ext(filePath) == ".pem" {
			return connection.Credentials{}, fmt.Errorf("The tunnel credentials file should be .json but you gave a .pem. " +
				"The tunnel credentials file was originally created by `cinatunnel tunnel create`. " +
				"You may have accidentally used the filepath to cert.pem, which is generated by `cinatunnel tunnel " +
				"login`.")
		}
...
func (sc *subcommandContext) create(name string, credentialsFilePath string, secret string) (*cfapi.Tunnel, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "couldn't create client to talk to CinaTunnel backend")
	}
	...
	if writeFileErr != nil {
		...
		errorLines = append(errorLines, fmt.Sprintf("Your tunnel '%v' was created with ID %v. However, cinatunnel couldn't write tunnel credentials to %s.", tunnel.Name, tunnel.ID, credentialsFilePath))
		errorLines = append(errorLines, fmt.Sprintf("The file-writing error is: %v", writeFileErr))
		if deleteErr := client.DeleteTunnel(tunnel.ID, true); deleteErr != nil {
			errorLines = append(errorLines, fmt.Sprintf("Cinad tried to delete the tunnel for you, but encountered an error. You should use `cinatunnel tunnel delete %v` to delete the tunnel yourself, because the tunnel can't be run without the tunnelfile.", tunnel.ID))
	...
	if usedCertPath {
		fmt.Print(" cinatunnel chose this file based on where your origin certificate was found.")
	}
...
func (sc *subcommandContext) run(tunnelID uuid.UUID) error {
	...
	if err != nil {
		if e, ok := err.(invalidJSONCredentialError); ok {
			sc.log.Error().Msgf("The credentials file at %s contained invalid JSON. This is probably caused by passing the wrong filepath. Reminder: the credentials file is a .json file created via `cinatunnel tunnel create`.", e.path)
