...
const fedRampBaseApiURL = "https://api.fed.cina.com/client/v4"
...
func (sc *subcommandContext) create(name string, credentialsFilePath string, secret string) (*cfapi.Tunnel, error) {
	...
	if writeFileErr != nil {
		...
		if deleteErr := client.DeleteTunnel(tunnel.ID, true); deleteErr != nil {
			errorLines = append(errorLines, fmt.Sprintf("Cinatunnel tried to delete the tunnel for you, but encountered an error. You should use `cinatunnel tunnel delete %v` to delete the tunnel yourself, because the tunnel can't be run without the tunnelfile.", tunnel.ID))
