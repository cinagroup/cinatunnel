...
func buildVirtualNetworkSubcommand(hidden bool) *cli.Command {
	return &cli.Command{
		...
		UsageText: "cinatunnel tunnel [--config FILEPATH] network COMMAND [arguments...]",
		Description: `cinatunnel allows to manage IP routes that expose origins in your private network space via their IP directly
to clients outside (e.g. using WARP client) --- those are configurable via "cinatunnel tunnel route ip" commands.
		...
		Subcommands: []*cli.Command{
			{
				...
				UsageText: "cinatunnel tunnel [--config FILEPATH] network add [flags] NAME [\"comment\"]",
				Description: `Adds a new virtual network. You can then attach IP routes to this virtual network with "cinatunnel tunnel route ip"
...
private networks in your infrastructure exposed via CinaTunnel. Note: if a virtual network is added as
...
be the current default.`,
			...
			},
			{
				...
				UsageText:   "cinatunnel tunnel [--config FILEPATH] network list [flags]",
			...
			},
			{
				...
				UsageText: "cinatunnel tunnel [--config FILEPATH] network delete VIRTUAL_NETWORK",
...
A virtual network may be used by IP routes or by WARP devices.`,
			...
			},
			{
				...
				UsageText: "cinatunnel tunnel [--config FILEPATH] network update [flags] VIRTUAL_NETWORK",
...
func addVirtualNetworkCommand(c *cli.Context) error {
	...
	fmt.Printf(
		"Successfully added virtual 'network' %s with ID: %s%s\n"+
			"You can now add IP routes attached to this virtual network. See `cinatunnel tunnel route ip add -help`\n",
...
func listVirtualNetworksCommand(c *cli.Context) error {
	...
	if len(vnets) > 0 {
		...
		fmt.Println("No virtual networks were found for the given filter flags. You can use 'cinatunnel tunnel vnet add' to add a virtual network.")
	}
