...
import (
	...
	"github.com/cinagroup/cinatunnel/prechecks"
...
const (
	//nolint:gosec // This is the Sentry DSN for cinatunnel which is safe to be public
...
For production usage, we recommend creating Named Tunnels. (https://github.com/cinagroup/cinatunnel)
`
)
...
var (
	...
	errDeprecatedClassicTunnel = errors.New("Classic tunnels have been deprecated, please use Named Tunnels. (https://github.com/cinagroup/cinatunnel)")
...
You can manage your Tunnels via one.dash.cina.com. This approach will only require you to run a single command
...
See https://github.com/cinagroup/cinatunnel for more info.`,
...
func StartServer(
	...
	// Run connectivity pre-checks for cinatunnel. This runs in a separate
	// goroutine, as we want to keep initializing cinatunnel while prechecks
	...
	go func() {
		...
		diagnosticHandler := diagnostic.NewDiagnosticHandler(
			log,
			0,
			diagnostic.NewSystemCollectorImpl(buildInfo.CinatunnelVersion),
...
func tunnelFlags(shouldHide bool) []cli.Flag {
	flags := configureCinatunnelFlags(shouldHide)
	...
	flags = append(flags, []cli.Flag{
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Value:   "https://api.cina.com/client/v4",
			Hidden:  true,
		}),
...
func configureCinatunnelFlags(shouldHide bool) []cli.Flag {
	return []cli.Flag{
		&cli.StringFlag{
			Name:   "config",
