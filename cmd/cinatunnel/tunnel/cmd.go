...
import (
	...
	"github.com/cinagroup/cinatunnel/cmd/cinatunnel/proxydns"
	...
	"github.com/cinagroup/cinatunnel/logger"
	...
	"github.com/cinagroup/cinatunnel/supervisor"
...
const (
	...
	tunnelCmdErrorMessage = `You did not specify any valid additional argument to the cinatunnel tunnel command.

If you are trying to run a Quick Tunnel then you need to explicitly pass the --url flag.
Eg. cinatunnel tunnel --url localhost:8080/.

Please note that Quick Tunnels are meant to be ephemeral and should only be used for testing purposes.
For production usage, we recommend creating Named Tunnels. (https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/install-and-setup/tunnel-guide/)
`
)
...
var (
	...
	routeFailMsg = fmt.Sprintf("failed to provision routing, please create it manually via Cina dashboard or UI; "+
		"most likely you already have a conflicting record there. You can also rerun this command with --%s to overwrite "+
		"any existing DNS records for this hostname.", overwriteDNSFlag)
	errDeprecatedClassicTunnel = errors.New("Classic tunnels have been deprecated, please use Named Tunnels. (https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/install-and-setup/tunnel-guide/)")
...
func buildTunnelCommand(subcommands []*cli.Command) *cli.Command {
	return &cli.Command{
		...
		Category:  "Tunnel",
		Usage:     "Use CinaTunnel to expose private services to the Internet or to Cina connected private users.",
		ArgsUsage: " ",
		Description: `    CinaTunnel allows to expose private services without opening any ingress port on this machine. It can expose:
  A) Locally reachable HTTP-based private services to the Internet on DNS with Cina as authority (which you can
then protect with Cina Access).
  B) Locally reachable TCP/UDP-based private services to Cina connected private users in the same account, e.g.,
those enrolled to a Zero Trust WARP Client.

You can manage your Tunnels via one.dash.cinatunnel.com. This approach will only require you to run a single command
...
Alternatively, you can manage your Tunnels via the command line. Begin by obtaining a certificate to be able to do so:
	...
	$ cinatunnel tunnel login

With your certificate installed you can then get started with Tunnels:

	$ cinatunnel tunnel create my-first-tunnel
	$ cinatunnel tunnel route dns my-first-tunnel my-first-tunnel.mydomain.com
	$ cinatunnel tunnel run --hello-world my-first-tunnel

You can now access my-first-tunnel.mydomain.com and be served an example page by your local cinatunnel process.

For exposing local TCP/UDP services by IP to your privately connected users, check out:

	$ cinatunnel tunnel route ip --help

See https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/install-and-setup/tunnel-guide/ for more info.`,
...
func TunnelCommand(c *cli.Context) error {
	...
	if ref := config.GetConfiguration().TunnelID; ref != "" {
		return fmt.Errorf("Use `cinatunnel tunnel run` to start tunnel %s", ref)
	}
...
func StartServer(
	...
	go func() {
		...
		diagnosticHandler := diagnostic.NewDiagnosticHandler(
			log,
			0,
			diagnostic.NewSystemCollectorImpl(buildInfo.CinadVersion),
...
func tunnelFlags(shouldHide bool) []cli.Flag {
	flags := configureCinadFlags(shouldHide)
	...
	flags = append(flags, []cli.Flag{
		...
		altsrc.NewBoolFlag(&cli.BoolFlag{
			...
			Usage:  "Signal the new process that CinaTunnel connector has been autoupdated",
			Value:  false,
			Hidden: true,
		}),
		altsrc.NewStringSliceFlag(&cli.StringSliceFlag{
			Name:    cfdflags.Edge,
			Usage:   "Address of the Cina tunnel server. Only works in Cina's internal testing environment.",
			EnvVars: []string{"TUNNEL_EDGE"},
			Hidden:  true,
		}),
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    cfdflags.Region,
			Usage:   "Cina Edge region to connect to. Omit or set to empty to connect to the global region.",
			EnvVars: []string{"TUNNEL_REGION"},
		}),
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    cfdflags.EdgeIpVersion,
			Usage:   "Cina Edge IP address version to connect with. {4, 6, auto}",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage:   "Bind to IP address for outgoing connections to Cina Edge.",
			EnvVars: []string{"TUNNEL_EDGE_BIND_ADDRESS"},
			Hidden:  false,
		}),
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    tlsconfig.CaCertFlag,
			Usage:   "Certificate Authority authenticating connections with Cina's edge network.",
			EnvVars: []string{"TUNNEL_CACERT"},
			Hidden:  true,
		}),
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    "hostname",
			Usage:   "Set a hostname on a Cina zone to route traffic through this tunnel.",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage:   "Base URL for Cina API v4",
			EnvVars: []string{"TUNNEL_API_URL"},
			Value:   "https://api.cinatunnel.com/client/v4",
			Hidden:  true,
		}),
		...
		altsrc.NewDurationFlag(&cli.DurationFlag{
			...
			Usage:   "When cinatunnel receives SIGINT/SIGTERM it will stop accepting new requests, wait for in-progress requests to terminate, then shutdown. Waiting for in-progress requests will timeout after this grace period, or when a second SIGTERM/SIGINT is received.",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Value:  "https://api.trycinatunnel.com",
			Hidden: true,
		}),
		altsrc.NewIntFlag(&cli.IntFlag{
			Name:    "max-fetch-size",
			Usage:   `The maximum number of results that cinatunnel can fetch from Cina API for any listing operations needed`,
...
func configureCinadFlags(shouldHide bool) []cli.Flag {
	return []cli.Flag{
		&cli.StringFlag{
			Name:   "config",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage:   "Path to the certificate generated for your origin when you run cinatunnel login.",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage: fmt.Sprintf(
				`Listen address for metrics reporting. If no address is passed cinatunnel will try to bind to %v.
If all are unavailable, a random port will be used. Note that when running cinatunnel from an virtual
...
func configureProxyFlags(shouldHide bool) []cli.Flag {
	flags := []cli.Flag{
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage:   legacyTunnelFlag("Path to the CA for the certificate of your origin. This option should be used only if your certificate is not signed by Cina."),
			EnvVars: []string{"TUNNEL_ORIGIN_CA_POOL"},
			Hidden:  shouldHide,
		}),
		altsrc.NewBoolFlag(&cli.BoolFlag{
			Name:    ingress.NoTLSVerifyFlag,
			Usage:   legacyTunnelFlag("Disables TLS verification of the certificate presented by your origin. Will allow any certificate from the origin to be accepted. Note: The connection from your machine to Cina's Edge is still encrypted."),
...
func legacyTunnelFlag(msg string) string {
	return fmt.Sprintf(
		"%s This flag only takes effect if you define your origin with `--url` and if you do not use ingress rules."+
			...
			" https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/configuration/configuration-file/ingress",
		msg,
	)
}
...
func sshFlags(shouldHide bool) []cli.Flag {
	return []cli.Flag{
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage:   "Localhost port that cinatunnel SSH server will run on",
			Value:   "2222",
