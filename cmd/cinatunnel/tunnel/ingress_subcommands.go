...
func buildIngressSubcommand() *cli.Command {
	return &cli.Command{
		Name:      "ingress",
		Category:  "Tunnel",
		Usage:     "Validate and test cinatunnel tunnel's ingress configuration",
		UsageText: "cinatunnel tunnel [--config FILEPATH] ingress COMMAND [arguments...]",
		Hidden:    true,
		Description: ` Cinatunnel lets you route traffic from the internet to multiple different addresses on your
		...
		To ensure cinatunnel can route all incoming requests, the last rule must be a catch-all
...
func buildValidateIngressCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:   "cinatunnel tunnel [--config FILEPATH] ingress validate",
...
func buildTestURLCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: "cinatunnel tunnel [--config FILEPATH] ingress rule URL",
		ArgsUsage: "URL",
...
// validateIngressCommand check the syntax of the ingress rules in the cinatunnel config file
...
func testURLCommand(c *cli.Context) error {
	...
	if requestArg == "" {
		return errors.New("cinatunnel tunnel rule expects a single argument, the URL to test")
	}
