...
import (
	...
	"github.com/cinagroup/cinatunnel/cmd/cinatunnel/updater"
	...
	"github.com/cinagroup/cinatunnel/metrics"
)
...
var (
	...
	selectProtocolFlag = altsrc.NewStringFlag(&cli.StringFlag{
		...
		Aliases: []string{"p"},
		Usage:   fmt.Sprintf("Protocol implementation to connect with Cina's edge network. %s", connection.AvailableProtocolFlagMessage),
	...
	cleanupClientFlag = &cli.StringFlag{
		Name:    "connector-id",
		Aliases: []string{"c"},
		Usage:   `Constraints the cleanup to stop the connections of a single Connector (by its ID). You can find the various Connectors (and their IDs) currently connected to your tunnel via 'cinatunnel tunnel info <name>'.`,
	...
	icmpv4SrcFlag = &cli.StringFlag{
		...
		Usage:   "Source address to send/receive ICMPv4 messages. If not provided cinatunnel will dial a local address to determine the source IP or fallback to 0.0.0.0.",
		EnvVars: []string{"TUNNEL_ICMPV4_SRC"},
	}
	icmpv6SrcFlag = &cli.StringFlag{
		Name:    flags.ICMPV6Src,
		Usage:   "Source address and the interface name to send/receive ICMPv6 messages. If not provided cinatunnel will dial a local address to determine the source IP or fallback to ::.",
	...
	maxActiveFlowsFlag = &cli.Uint64Flag{
		...
		Usage:   "Overrides the remote configuration for max active private network flows (TCP/UDP) that this cinatunnel instance supports",
...
func buildCreateCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: "cinatunnel tunnel [tunnel command options] create [subcommand options] NAME",
		Description: `Creates a tunnel, registers it with Cina edge and generates credential file used to run this tunnel.
  Use "cinatunnel tunnel route" subcommand to map a DNS name to this tunnel and "cinatunnel tunnel run" to start the connection.

  For example, to create a tunnel named 'my-tunnel' run:

  $ cinatunnel tunnel create my-tunnel`,
...
func createCommand(c *cli.Context) error {
	...
	if c.NArg() != 1 {
		return cliutil.UsageError(`"cinatunnel tunnel create" requires exactly 1 argument, the name of tunnel to create.`)
	}
	name := c.Args().First()
...
func buildListCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:   "cinatunnel tunnel [tunnel command options] list [subcommand options]",
		Description: "cinatunnel tunnel list will display all active tunnels, their created time and associated connections. Use -d flag to include deleted tunnels. See the list of options to filter the list",
		Flags: []cli.Flag{
			outputFormatFlag,
			showDeletedFlag,
			listNameFlag,
...
func listCommand(c *cli.Context) error {
	...
	if len(tunnels) > 0 {
		...
		fmt.Println("No tunnels were found for the given filter flags. You can use 'cinatunnel tunnel create' to create a tunnel.")
	}
...
func formatAndPrintTunnelList(tunnels []*cfapi.Tunnel, showRecentlyDisconnected bool) {
	...
	_, _ = fmt.Fprintln(writer, "You can obtain more detailed information for each tunnel with `cinatunnel tunnel info <name/uuid>`")
...
func buildReadyCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:          "cinatunnel tunnel [tunnel command options] ready [subcommand options]",
		Description:        "cinatunnel tunnel ready will return proper exit code based on the /ready endpoint",
		Flags:              []cli.Flag{},
		CustomHelpTemplate: commandHelpTemplate(),
	}
}
...
func buildInfoCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:   "cinatunnel tunnel [tunnel command options] info [subcommand options] [TUNNEL]",
		Description: "cinatunnel tunnel info displays details about the active connectors for a given tunnel (identified by name or uuid).",
		Flags: []cli.Flag{
			outputFormatFlag,
			showRecentlyDisconnected,
			sortInfoByFlag,
...
func tunnelInfo(c *cli.Context) error {
	...
	if c.NArg() != 1 {
		return cliutil.UsageError(`"cinatunnel tunnel info" accepts exactly one argument, the ID or name of the tunnel to get info about.`)
	}
...
func buildDeleteCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:          "cinatunnel tunnel [tunnel command options] delete [subcommand options] TUNNEL",
		Description:        "cinatunnel tunnel delete will delete tunnels with the given tunnel UUIDs or names. A tunnel cannot be deleted if it has active connections. To delete the tunnel unconditionally, use -f flag.",
...
func deleteCommand(c *cli.Context) error {
	...
	if c.NArg() < 1 {
		return cliutil.UsageError(`"cinatunnel tunnel delete" requires at least 1 argument, the ID or name of the tunnel to delete.`)
	}
...
func buildRunCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: "cinatunnel tunnel [tunnel command options] run [subcommand options] [TUNNEL]",
		Description: `Runs the tunnel identified by name or UUID, creating highly available connections
  between your server and the Cina edge. You can provide name or UUID of tunnel to run either as the
  last command line argument or in the configuration file using "tunnel: TUNNEL".

  This command requires the tunnel credentials file created when "cinatunnel tunnel create" was run,
  however it does not need access to cert.pem from "cinatunnel login" if you identify the tunnel by UUID.
  If you experience other problems running the tunnel, "cinatunnel tunnel cleanup" may help by removing
...
func runCommand(c *cli.Context) error {
	...
	if c.NArg() > 1 {
		return cliutil.UsageError(`"cinatunnel tunnel run" accepts only one argument, the ID or name of the tunnel to run.`)
	}

	if c.String("hostname") != "" {
		sc.log.Warn().Msg("The property `hostname` in your configuration is ignored because you configured a Named Tunnel " +
			"in the property `tunnel` to run. Make sure to provision the routing (e.g. via `cinatunnel tunnel route dns/lb`) or else " +
	...
	if tokenStr != "" {
		...
		if tunnelRef == "" {
			...
			if tunnelRef == "" {
				return cliutil.UsageError(`"cinatunnel tunnel run" requires the ID or name of the tunnel to run as the last command line argument or in the configuration file.`)
			}
		}
...
func buildCleanupCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:          "cinatunnel tunnel [tunnel command options] cleanup [subcommand options] TUNNEL",
...
func cleanupCommand(c *cli.Context) error {
	if c.NArg() < 1 {
		return cliutil.UsageError(`"cinatunnel tunnel cleanup" requires at least 1 argument, the IDs of the tunnels to cleanup connections.`)
	}
...
func buildTokenCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:          "cinatunnel tunnel [tunnel command options] token [subcommand options] TUNNEL",
		Description:        "cinatunnel tunnel token will fetch the credentials token for a given tunnel (by its name or UUID), which is then used to run the tunnel. This command fails if the tunnel does not exist or has been deleted. Use the flag `cinatunnel tunnel token --cred-file /my/path/file.json TUNNEL` to output the token to the credentials JSON file. Note: this command only works for Tunnels created since cinatunnel version 2022.3.0",
...
func tokenCommand(c *cli.Context) error {
	...
	if c.NArg() != 1 {
		return cliutil.UsageError(`"cinatunnel tunnel token" requires exactly 1 argument, the name or UUID of tunnel to fetch the credentials token for.`)
	}
	tunnelID, err := sc.findID(c.Args().First())
	if err != nil {
		return errors.Wrap(err, "error parsing tunnel ID")
...
func buildRouteCommand() *cli.Command {
	return &cli.Command{
		Name:      "route",
		Usage:     "Define which traffic routed from Cina edge to this tunnel: requests to a DNS hostname, to a Cina Load Balancer, or traffic originating from Cina WARP clients",
		UsageText: "cinatunnel tunnel [tunnel command options] route [subcommand options] [dns TUNNEL HOSTNAME]|[lb TUNNEL HOSTNAME LB-POOL]|[ip NETWORK TUNNEL]",
		Description: `The route command defines how Cina will proxy requests to this tunnel.

To route a hostname by creating a DNS CNAME record to a tunnel:
   cinatunnel tunnel route dns <tunnel ID or name> <hostname>
You can read more at: https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/routing-to-tunnel/dns

To use this tunnel as a load balancer origin, creating pool and load balancer if necessary:
   cinatunnel tunnel route lb <tunnel ID or name> <hostname> <load balancer pool>
You can read more at: https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/routing-to-tunnel/lb

For Cina WARP traffic to be routed to your private network, reachable from this tunnel as origins, use:
   cinatunnel tunnel route ip <network CIDR> <tunnel ID or name>
Further information about managing Cina WARP traffic to your tunnel is available at:
   cinatunnel tunnel route ip --help
`,
		CustomHelpTemplate: commandHelpTemplate(),
		Subcommands: []*cli.Command{
			{
				...
				UsageText:   "cinatunnel tunnel route dns [TUNNEL] [HOSTNAME]",
			...
			},
			{
				...
				UsageText:   "cinatunnel tunnel route lb [TUNNEL] [HOSTNAME] [LB-POOL-NAME]",
...
func routeDnsCommand(c *cli.Context) error {
	if c.NArg() != 2 {
		return cliutil.UsageError(`This command expects the format "cinatunnel tunnel route dns <tunnel name/id> <hostname>"`)
	}
...
func routeLbCommand(c *cli.Context) error {
	if c.NArg() != 3 {
		return cliutil.UsageError(`This command expects the format "cinatunnel tunnel route lb <tunnel name/id> <hostname> <load balancer pool>"`)
	}
...
func commandHelpTemplate() string {
	...
	for _, f := range configureCinadFlags(false) {
...
func buildDiagCommand() *cli.Command {
	return &cli.Command{
		...
		Usage:       "Creates a diagnostic report from a local cinatunnel instance",
		UsageText:   "cinatunnel tunnel [tunnel command options] diag [subcommand options]",
		Description: "cinatunnel tunnel diag will create a diagnostic report of a local cinatunnel instance. The diagnostic procedure collects: logs, metrics, system information, traceroute to Cina Edge, and runtime information. Since there may be multiple instances of cinatunnel running the --metrics option may be provided to target a specific instance.",
		Flags: []cli.Flag{
			metricsFlag,
			diagContainerFlag,
			diagPodFlag,
