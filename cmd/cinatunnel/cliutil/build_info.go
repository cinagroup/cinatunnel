...
type BuildInfo struct {
	...
	CinatunnelVersion string `json:"cinatunnel_version"`
...
func GetBuildInfo(buildType, version string) *BuildInfo {
	return &BuildInfo{
		...
		BuildType:          buildType,
		CinatunnelVersion: version,
		Checksum:           currentBinaryChecksum(),
	}
}

func (bi *BuildInfo) Log(log *zerolog.Logger) {
	log.Info().Msgf("Version %s (Checksum %s)", bi.CinatunnelVersion, bi.Checksum)
	if bi.BuildType != "" {
...
func (bi *BuildInfo) UserAgent() string {
	return fmt.Sprintf("cinatunnel/%s", bi.CinatunnelVersion)
}
