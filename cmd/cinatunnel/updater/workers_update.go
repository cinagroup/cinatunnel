...
const (
	...
	// stop the service
	// rename cinatunnel.exe to cinatunnel.exe.old
	// rename cinatunnel.exe.new to cinatunnel.exe
	// delete cinatunnel.exe.old
	// start the service
	// exit with code 0 if we've reached this point indicating success.
	windowsUpdateCommandTemplate = `sc stop cinatunnel >nul 2>&1
del "{{.OldPath}}"
rename "{{.TargetPath}}" {{.OldName}}
rename "{{.NewPath}}" {{.BinaryName}}
sc start cinatunnel >nul 2>&1
exit /b 0`
...
// target path is where the file should be replace. Normally this the running cinatunnel's path
// userMessage is a possible message to convey back to the user after having checked in with the Updater Service
// isCompressed tells whether the asset to update cinatunnel is compressed or not
...
func (v *WorkersVersion) Apply() error {
	...
	// the easiest way to do this is with a batch file (or with a DLL, but that gets ugly for a cross compiled binary like cinatunnel)
