...
const (
	...
	noUpdateInShellMessage        = "cinatunnel will not automatically update when run from the shell. To enable auto-updates, run cinatunnel as a service: https://github.com/cinagroup/cinatunnel"
...
func CheckForUpdate(options updateOptions) (CheckResult, error) {
	...
	s := NewWorkersService(buildInfo.CinatunnelVersion, url, cfdPath, Options{IsBeta: options.isBeta,
...
func (a *AutoUpdater) Run(ctx context.Context) error {
	...
	for {
		...
		if updateOutcome.Updated {
			buildInfo.CinatunnelVersion = updateOutcome.Version
			if IsSysV() {
