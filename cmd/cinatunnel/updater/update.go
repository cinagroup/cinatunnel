...
const (
	...
	noUpdateInShellMessage        = "cinatunnel will not automatically update when run from the shell. To enable auto-updates, run cinatunnel as a service: https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/configure-tunnels/local-management/as-a-service/"
	noUpdateOnWindowsMessage      = "cinatunnel will not automatically update on Windows systems."
	noUpdateManagedPackageMessage = "cinatunnel will not automatically update if installed by a package manager."
...
func (u *statusSuccess) Error() string {
	return fmt.Sprintf("cinatunnel has been updated to version %s", u.newVersion)
}
...
func (e *statusError) Error() string {
	return fmt.Sprintf("failed to update cinatunnel: %v", e.err)
}
...
func CheckForUpdate(options updateOptions) (CheckResult, error) {
	...
	s := NewWorkersService(buildInfo.CinadVersion, url, cfdPath, Options{IsBeta: options.isBeta,
...
func Update(c *cli.Context) error {
	...
	if wasInstalledFromPackageManager() {
		...
		log.Error().Msg(fmt.Sprintf("cinatunnel was installed by %s. Please update using the same method.", packageManagerName))
		return nil
	}

	isBeta := c.Bool("beta")
	if isBeta {
		log.Info().Msg("cinatunnel is set to update to the latest beta version")
	}

	isStaging := c.Bool("staging")
	if isStaging {
		log.Info().Msg("cinatunnel is set to update from staging")
	}

	isForced := c.Bool(cfdflags.Force)
	if isForced {
		log.Info().Msg("cinatunnel is set to upgrade to the latest publish version regardless of the current version")
	}
	...
	if updateOutcome.noUpdate() {
		log.Info().Str(LogFieldVersion, updateOutcome.Version).Msg("cinatunnel is up to date")
		return nil
	}
...
func loggedUpdate(log *zerolog.Logger, options updateOptions) UpdateOutcome {
	...
	if updateOutcome.Updated {
		log.Info().Str(LogFieldVersion, updateOutcome.Version).Msg("cinatunnel has been updated")
	}
	if updateOutcome.Error != nil {
...
// AutoUpdater periodically checks for new version of cinatunnel.
...
// Run will perodically check for cinatunnel updates, download them, and then restart the current cinatunnel process
...
func (a *AutoUpdater) Run(ctx context.Context) error {
	...
	for {
		...
		if updateOutcome.Updated {
			buildInfo.CinadVersion = updateOutcome.Version
			if IsSysV() {
