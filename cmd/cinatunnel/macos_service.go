...
const (
	launchdIdentifier = "com.cina.cinatunnel"
)

func runApp(app *cli.App, _ chan struct{}) {
	app.Commands = append(app.Commands, &cli.Command{
		Name:  "service",
		Usage: "Manages the cinatunnel launch agent",
		Subcommands: []*cli.Command{
			{
				Name:   "install",
				Usage:  "Install cinatunnel as an user launch agent",
				Action: cliutil.ConfiguredAction(installLaunchd),
			},
			{
				Name:   "uninstall",
				Usage:  "Uninstall the cinatunnel launch agent",
...
func installLaunchd(c *cli.Context) error {
	...
	if isRootUser() {
		log.Info().Msg("Installing cinatunnel client as a system launch daemon. " +
			"cinatunnel client will run at boot")
	} else {
		log.Info().Msg("Installing cinatunnel client as an user launch agent. " +
			"Note that cinatunnel client will only run when the user is logged in. " +
			"If you want to run cinatunnel client at boot, install with root permission. " +
			"For more information, visit https://docs.cinatunnel.com/cloudflare-one/connections/connect-networks/configure-tunnels/local-management/as-a-service/macos/")
	}
	...
	if err == nil {
		log.Info().Msg("MacOS service for cinatunnel installed successfully")
	}
	return err
}
...
func uninstallLaunchd(c *cli.Context) error {
	...
	if isRootUser() {
		log.Info().Msg("Uninstalling cinatunnel as a system launch daemon")
	} else {
		log.Info().Msg("Uninstalling cinatunnel as a user launch agent")
	}
	installPath, err := installPath()
	if err != nil {
		return errors.Wrap(err, "error determining install path")
	...
	if err == nil {
		log.Info().Msg("Launchd for cinatunnel was uninstalled successfully")
	}
	return err
}
