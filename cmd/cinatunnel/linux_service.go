...
import (
	...
	"github.com/cinagroup/cinatunnel/cmd/cinatunnel/tunnel"
...
func runApp(app *cli.App, _ chan struct{}) {
	app.Commands = append(app.Commands, &cli.Command{
		Name:  "service",
		Usage: "Manages the cinatunnel system service",
		Subcommands: []*cli.Command{
			{
				Name:   "install",
				Usage:  "Install cinatunnel as a system service",
			...
			{
				Name:   "uninstall",
				Usage:  "Uninstall the cinatunnel service",
...
const (
	serviceConfigDir         = "/etc/cinatunnel"
	serviceConfigFile        = "config.yml"
	serviceCredentialFile    = "cert.pem"
	serviceConfigPath        = serviceConfigDir + "/" + serviceConfigFile
	cinatunnelService       = "cinatunnel.service"
	cinatunnelUpdateService = "cinatunnel-update.service"
	cinatunnelUpdateTimer   = "cinatunnel-update.timer"
)

var systemdAllTemplates = map[string]ServiceTemplate{
	cinatunnelService: {
		Path: fmt.Sprintf("/etc/systemd/system/%s", cinatunnelService),
		Content: `[Unit]
Description=cinatunnel
...
`,
	...
	cinatunnelUpdateService: {
		Path: fmt.Sprintf("/etc/systemd/system/%s", cinatunnelUpdateService),
		Content: `[Unit]
Description=Update cinatunnel
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/bin/bash -c '{{ .Path }} update; code=$?; if [ $code -eq 11 ]; then systemctl restart cinatunnel; exit 0; fi; exit $code'
`,
	},
	cinatunnelUpdateTimer: {
		Path: fmt.Sprintf("/etc/systemd/system/%s", cinatunnelUpdateTimer),
		Content: `[Unit]
Description=Update cinatunnel
...
# Default-Stop:      0 1 6
# Short-Description: cinatunnel
# Description:       cinatunnel agent
### END INIT INFO
...
var noUpdateServiceFlag = &cli.BoolFlag{
	...
	Usage: "Disable auto-update of the cinatunnel linux service, which restarts the server to upgrade for new versions.",
	Value: false,
}
...
func installLinuxService(c *cli.Context) error {
	...
	if err == nil {
		log.Info().Msg("Linux service for cinatunnel installed successfully")
	}
	return err
}
...
`, src.Source())
	...
	if src.Source() != serviceConfigPath {
		if exists, err := config.FileExists(serviceConfigPath); err != nil || exists {
			return nil, fmt.Errorf("Possible conflicting configuration in %[1]s and %[2]s. Either remove %[2]s or run `cinatunnel --config %[2]s service install`", src.Source(), serviceConfigPath)
		}
	...
	return []string{
		"--config", "/etc/cinatunnel/config.yml", "tunnel", "run",
	}, nil
}
...
func installSystemd(templateArgs *ServiceTemplateArgs, autoUpdate bool, log *zerolog.Logger) error {
	...
	if autoUpdate {
		systemdTemplates = []ServiceTemplate{
			systemdAllTemplates[cinatunnelService],
			systemdAllTemplates[cinatunnelUpdateService],
			systemdAllTemplates[cinatunnelUpdateTimer],
		}
	} else {
		systemdTemplates = []ServiceTemplate{
			systemdAllTemplates[cinatunnelService],
	...
	if err := runCommand("systemctl", "enable", cinatunnelService); err != nil {
		log.Err(err).Msgf("systemctl enable %s error", cinatunnelService)
		return err
	}

	if autoUpdate {
		if err := runCommand("systemctl", "start", cinatunnelUpdateTimer); err != nil {
			log.Err(err).Msgf("systemctl start %s error", cinatunnelUpdateTimer)
			return err
		}
	}
	...
	return runCommand("systemctl", "start", cinatunnelService)
}
...
func installSysv(templateArgs *ServiceTemplateArgs, autoUpdate bool, log *zerolog.Logger) error {
	...
	return runCommand("service", "cinatunnel", "start")
}
...
func uninstallLinuxService(c *cli.Context) error {
	...
	if err == nil {
		log.Info().Msg("Linux service for cinatunnel uninstalled successfully")
	}
	return err
}
...
func uninstallSystemd(log *zerolog.Logger) error {
	...
	if _, exists := installedServices[cinatunnelService]; exists {
		if err := runCommand("systemctl", "disable", cinatunnelService); err != nil {
			log.Err(err).Msgf("systemctl disable %s error", cinatunnelService)
			return err
		}
		if err := runCommand("systemctl", "stop", cinatunnelService); err != nil {
			log.Err(err).Msgf("systemctl stop %s error", cinatunnelService)
			return err
		}
	}

	if _, exists := installedServices[cinatunnelUpdateTimer]; exists {
		if err := runCommand("systemctl", "stop", cinatunnelUpdateTimer); err != nil {
			log.Err(err).Msgf("systemctl stop %s error", cinatunnelUpdateTimer)
			return err
		}
	}
...
func uninstallSysv(log *zerolog.Logger) error {
	if err := runCommand("service", "cinatunnel", "stop"); err != nil {
		log.Err(err).Msg("service cinatunnel stop error")
		return err
	}
