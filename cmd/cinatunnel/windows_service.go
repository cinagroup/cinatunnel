...
const (
	windowsServiceName        = "Cinad"
	windowsServiceDescription = "Cinad agent"
	windowsServiceUrl         = "https://docs.cinatunnel.com/cloudflare-one/connections/connect-apps/configure-tunnels/local-management/as-a-service/windows/"
...
func runApp(app *cli.App, graceShutdownC chan struct{}) {
	app.Commands = append(app.Commands, &cli.Command{
		Name:  "service",
		Usage: "Manages the cinatunnel Windows service",
		Subcommands: []*cli.Command{
			{
				Name:   "install",
				Usage:  "Install cinatunnel as a Windows service",
	...
	// like running cinatunnel in a GCP startup script, for which
...
func (s *windowsService) Execute(serviceArgs []string, r <-chan svc.ChangeRequest, statusChan chan<- svc.Status) (ssec bool, errno uint32) {
	...
	for {
		...
		case c := <-r:
			...
			case svc.Stop, svc.Shutdown:
				if s.graceShutdownC != nil {
					...
					elog.Info(1, "cinatunnel starting graceful shutdown")
				...
				elog.Info(1, "cinatunnel terminating immediately")
				statusChan <- svc.Status{State: svc.StopPending}
				return false, 0
			default:
		...
		case err := <-errC:
			if err != nil {
				elog.Error(1, fmt.Sprintf("cinatunnel terminated with error %v", err))
				ssec = true
				errno = 1
			} else {
				elog.Info(1, "cinatunnel terminated without error")
				errno = 0
			}
			return
		}
...
func installWindowsService(c *cli.Context) error {
	...
	zeroLogger.Info().Msg("Installing cinatunnel Windows service")
	...
	defer s.Close()
	log.Info().Msg("cinatunnel agent service is installed")
	...
	err = s.Start()
	if err == nil {
		log.Info().Msg("Agent service for cinatunnel installed successfully")
	}
	return err
}
...
func uninstallWindowsService(c *cli.Context) error {
	...
	log.Info().Msg("Uninstalling cinatunnel agent service")
	m, err := mgr.Connect()
	if err != nil {
		return errors.Wrap(err, "Cannot establish a connection to the service control manager")
	}
	...
	if status, err := s.Query(); err == nil && status.State == svc.Running {
		log.Info().Msg("Stopping cinatunnel agent service")
		if _, err := s.Control(svc.Stop); err != nil {
			log.Info().Err(err).Msg("Failed to stop cinatunnel agent service, you may need to stop it manually to complete uninstall.")
		}
	}
	...
	log.Info().Msg("Agent service for cinatunnel was uninstalled successfully")
