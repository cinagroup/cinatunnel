...
# But we keep the binary installed by it to be named "cinatunnel" regardless.
define build_package
	mkdir -p $(PACKAGE_DIR)
	cp cinatunnel $(PACKAGE_DIR)/cinatunnel
	cp cinatunnel.1 $(PACKAGE_DIR)/cinatunnel.1
	fpm -C $(PACKAGE_DIR) -s dir -t $(1) \
		--description 'CinaTunnel daemon' \
		--vendor 'Cina' \
		--license 'Apache License Version 2.0' \
		--url 'https://github.com/cinagroup/cinatunnel' \
		-m 'Cina <support@cinatunnel.com>' \
	    -a $(PACKAGE_ARCH) -v $(VERSION) -n $(DEB_PACKAGE_NAME) $(RPM_DIGEST) $(NIGHTLY_FLAGS) --after-install postinst.sh --after-remove postrm.sh \
		cinatunnel=$(INSTALL_BINDIR) cinatunnel.1=$(INSTALL_MANDIR)
endef

.PHONY: cinatunnel-deb
cinatunnel-deb: cinatunnel cinatunnel.1
	$(call build_package,deb)

.PHONY: cinatunnel-rpm
cinatunnel-rpm: cinatunnel cinatunnel.1
	$(call build_package,rpm)

.PHONY: cinatunnel-msi
cinatunnel-msi:
	wixl --define Version=$(VERSION) --define Path=$(EXECUTABLE_PATH) --output cinatunnel-$(VERSION)-$(TARGET_ARCH).msi cinatunnel.wxs
...
.PHONY: vet
vet:
	$Q go vet -mod=vendor github.com/cinagroup/cinatunnel/...
...
.PHONY: ci-build
ci-build:
	@GOOS=linux GOARCH=amd64 $(MAKE) cinatunnel
	@mkdir -p artifacts
	@mv cinatunnel artifacts/cinatunnel

.PHONY: ci-fips-build
ci-fips-build:
	@FIPS=true GOOS=linux GOARCH=amd64 $(MAKE) cinatunnel
	@mkdir -p artifacts
	@mv cinatunnel artifacts/cinatunnel
