...
class TestTermination:
    ...
    def test_graceful_shutdown(self, tmp_path, component_tests_config, signal, protocol):
        ...
        with start_cinatunnel(
                tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1"],  new_process=True, capture_output=False) as cinatunnel:
            ...
            with self.within_grace_period():
                self.terminate_by_signal(cinatunnel, signal)
                self.wait_eyeball_thread(
                    in_flight_req, self.grace_period + self.timeout)

    # test cinatunnel terminates before grace period expires when all eyeball
    ...
    def terminate_by_signal(self, cinatunnel, sig):
        cinatunnel.send_signal(sig)
    ...
    def within_grace_period(self, has_connection=True):
        try:
            ...
            # If the request fell within the grace period cinatunnel can close earlier, but to ensure that it doesn't
            # close immediately we add a minimum boundary. If cinatunnel shutdown in less than 1s it's likely that
            # it shutdown as soon as it received SIGINT. The only way cinatunnel can close immediately is if it has no
            # in-flight requests
