...
from cli import CinadCli
from util import LOGGER, write_config, start_cinatunnel, wait_tunnel_ready, send_requests, decode_jwt_payload
import platform

"""
Each test in TestManagement will:
1. Acquire a management token from Cina public API
2. Make a request against the management service for the running tunnel
"""
class TestManagement:
    """
        test_get_host_details does the following:
        1. It gets a management token from Tunnelstore using cinatunnel tail token <tunnel_id>
    ...
    def test_get_host_details(self, tmp_path, component_tests_config):
        ...
        with start_cinatunnel(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1", "--label" , "test"], cfd_args=["run", "--hello-world"], new_process=True):
            wait_tunnel_ready(tunnel_url=config.get_url(),
                              require_min_connections=1)
            cfd_cli = CinadCli(config, config_path, LOGGER)
            ...
            assert resp.status_code == 200, "Expected cinatunnel to return 200 for host details"
            assert resp.json()["hostname"] == "custom:test", "Expected cinatunnel to return hostname"
            assert resp.json()["ip"] != "", "Expected cinatunnel to return ip"
            assert resp.json()["connector_id"] == connector_id, "Expected cinatunnel to return connector_id"
    ...
    def test_get_metrics(self, tmp_path, component_tests_config):
        ...
        LOGGER.debug(config)
        config_path = write_config(tmp_path, config.full_config)
        with start_cinatunnel(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1"], new_process=True):
            ...
            # Assert response.
            assert resp.status_code == 200, "Expected cinatunnel to return 200 for /metrics"
    ...
    def test_get_pprof_heap(self, tmp_path, component_tests_config):
        ...
        with start_cinatunnel(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1"], new_process=True):
            ...
            # Assert response.
            assert resp.status_code == 200, "Expected cinatunnel to return 200 for /debug/pprof/heap"
    ...
    def test_get_metrics_when_disabled(self, tmp_path, component_tests_config):
        ...
        LOGGER.debug(config)
        config_path = write_config(tmp_path, config.full_config)
        with start_cinatunnel(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1", "--management-diagnostics=false"], new_process=True):
            wait_tunnel_ready(require_min_connections=1)
            cfd_cli = CinadCli(config, config_path, LOGGER)
            url = cfd_cli.get_management_url("metrics", config, config_path, resource="admin")
            resp = send_request(url)
            ...
            # Assert response.
            assert resp.status_code == 404, "Expected cinatunnel to return 404 for /metrics"

    def test_tail_token_command(self, tmp_path, component_tests_config):
        """
        Validates that 'cinatunnel tail token' command returns a token 
