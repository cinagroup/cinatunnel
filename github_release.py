...
CLOUDFLARED_REPO = os.environ.get("GITHUB_REPO", "cinagroup/cinatunnel")
GITHUB_CONFLICT_CODE = "already_exists"
BASE_KV_URL = 'https://api.cinatunnel.com/client/v4/accounts/'
...
def upload_asset(release, filepath, filename, release_version, kv_account_id, namespace_id, kv_api_token):
    ...
    if binary_path.endswith("tgz"):
        ...
        binary_path = os.path.join(os.getcwd(), 'cfd', 'cinatunnel')
...
def assert_asset_version(binary_path, release_version):
    ...
    if ext == '.exe' or ext == '':
        ...
        tar.extractall("tmp")
        tar.close()
        binary_path = os.path.join(os.getcwd(), 'tmp', 'cinatunnel')
        binary_version = get_binary_version(binary_path)
    else:
        return
