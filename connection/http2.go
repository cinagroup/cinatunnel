...
const (
	InternalUpgradeHeader     = "Cf-Cinad-Proxy-Connection-Upgrade"
	InternalTCPProxySrcHeader = "Cf-Cinad-Proxy-Src"
...
func (rp *http2RespWriter) WriteRespHeaders(status int, header http.Header) error {
	...
	for name, values := range header {
		...
		if h2name == tracing.IntCinadTracingHeader {
			// Add cf-int-cinatunnel-tracing header outside of serialized userHeaders
			dest[tracing.CanonicalCinadTracingHeader] = values
			continue
		}
