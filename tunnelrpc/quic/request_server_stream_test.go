...
func TestRegisterUdpSession(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			go func() {
				ss := NewCinadServer(nil, test.sessionRPCServer, nil, 10*time.Second)
			...
			rpcClientStream, err := NewCinadClient(t.Context(), clientStream, 5*time.Second)
			require.NoError(t, err)
...
func TestManageConfiguration(t *testing.T) {
	...
	go func() {
		server := NewCinadServer(nil, nil, configRPCServer, 10*time.Second)
	...
	defer cancel()
	rpcClientStream, err := NewCinadClient(ctx, clientStream, 5*time.Second)
	require.NoError(t, err)
